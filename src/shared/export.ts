import { getAllNotes, getNotesForVideo } from "./storage";
...
export async function exportNotesForVideo(videoId: string): Promise<void> {
	...
	downloadMarkdown(
		markdown,
		`${sanitizeFilename(firstNote.videoTitle)}-notes.md`,
	);
}

export async function exportAllNotes(): Promise<void> {
	const allNotes = await getAllNotes();
	const entries = Object.entries(allNotes).filter(
	...
	downloadMarkdown(
		markdown,
		`vidpulse-notes-${new Date().toISOString().split("T")[0]}.md`,
	);
}
