...
export async function setCache(videoId: string, analysis: VideoAnalysis): Promise<void> {
  const settings = await getSettings();
  const key = `${CACHE_PREFIX}${videoId}`;
...
async function incrementPreferencesVersion(): Promise<void> {
  const settings = await getSettings();
  await chrome.storage.sync.set({
    settings: { ...settings, preferencesVersion: settings.preferencesVersion + 1 }
  });
}
...
export async function updateMemory(memoryId: string, updates: Partial<MemoryEntry>): Promise<void> {
  const memories = await getMemories();
...
export async function replaceMemories(newMemories: MemoryEntry[]): Promise<void> {
...
export async function getFeedbackForVideo(videoId: string): Promise<VideoFeedback | null> {
  const history = await getFeedbackHistory();
...
export async function setSessionIntent(intent: SessionData['intent']): Promise<void> {
  const session = await getSession();
  if (session) {
...
export async function updateChannelStats(channelId: string, channelName: string, scores: { productivity: number; educational: number; entertainment: number; inspiring: number; creative: number }): Promise<ChannelStats> {
  ...
  if (existing) {
    ...
    existing.avgScores.productivity = Math.round(existing.totalScores.productivity / existing.videoCount);
    existing.avgScores.educational = Math.round(existing.totalScores.educational / existing.videoCount);
    existing.avgScores.entertainment = Math.round(existing.totalScores.entertainment / existing.videoCount);
    existing.avgScores.inspiring = Math.round(existing.totalScores.inspiring / existing.videoCount);
    existing.avgScores.creative = Math.round(existing.totalScores.creative / existing.videoCount);
...
export async function setChannelTrust(channelId: string, trust: 'trusted' | 'blocked' | null): Promise<void> {
  const allStats = await getChannelStats();
...
export async function updateDailyStats(
  ...
  const maxScore = Math.max(scores.productivity, scores.educational, scores.entertainment, scores.inspiring, scores.creative);
  const category = scores.educational === maxScore ? 'educational'
    : scores.entertainment === maxScore ? 'entertainment'
    : scores.inspiring === maxScore ? 'inspiring'
    : scores.creative === maxScore ? 'creative'
    : 'productive';
  ...
  if (!allStats[key]) {
    allStats[key] = {
      ...
      avgScores: { productivity: 0, educational: 0, entertainment: 0, inspiring: 0, creative: 0 },
      guardianBlocks: 0,
      guardianOverrides: 0,
      channelsWatched: [],
    };
  ...
  stats.avgScores.productivity = Math.round(((stats.avgScores.productivity * (n - 1)) + scores.productivity) / n);
  stats.avgScores.educational = Math.round(((stats.avgScores.educational * (n - 1)) + scores.educational) / n);
  stats.avgScores.entertainment = Math.round(((stats.avgScores.entertainment * (n - 1)) + scores.entertainment) / n);
  stats.avgScores.inspiring = Math.round(((stats.avgScores.inspiring * (n - 1)) + scores.inspiring) / n);
  stats.avgScores.creative = Math.round(((stats.avgScores.creative * (n - 1)) + scores.creative) / n);
...
export async function saveFocusSchedule(schedule: Partial<FocusSchedule>): Promise<void> {
  ...
  await chrome.storage.sync.set({ [FOCUS_SCHEDULE_KEY]: { ...current, ...schedule } });
}
...
export function isInFocusPeriod(schedule: FocusSchedule): boolean {
  ...
  return schedule.periods.some(period =>
    period.days.includes(day) && hour >= period.startHour && hour < period.endHour
  );
}
...
export async function getAllNotesIndex(): Promise<NotesIndex[]> {
  ...
  for (const [videoId, notes] of Object.entries(allNotes)) {
    if (notes.length > 0) {
      const lastNote = notes.reduce((a, b) => a.updatedAt > b.updatedAt ? a : b);
      index.push({
        videoId,
