...
import type { VideoAnalysis, MemoryEntry, Settings, KeyPoint, SessionData, ChannelStats, FocusSchedule, RelatedResource, RelatedContentCache, VideoSource } from '../shared/types';
import type {
  Message,
...
Respond with ONLY the summary text, no formatting or labels.`;
  ...
  const result = await generateText(genAI, settings.models.summarization, prompt);
...
Extract 3-8 key points depending on video length and complexity.`;
  ...
  return generateJson<KeyPoint[]>(genAI, settings.models.summarization, prompt, []);
}
...
Respond with ONLY a JSON array of tags, e.g.: ["tech", "productivity", "tutorial"]`;
  ...
  return generateJson<string[]>(genAI, settings.models.tagGeneration, prompt, ['untagged']);
}
...
async function analyzeContent(
  ...
  const hasPersonalization = settings.aboutMe && settings.aboutMe.trim().length > 0;
...
Respond with ONLY valid JSON:
{
  ...
  const defaultResult: AnalysisResult = {
    scores: { productivity: 0, entertainment: 0, educational: 0, inspiring: 0, creative: 0 },
    verdict: 'skip',
  };
  return generateJson<AnalysisResult>(genAI, settings.models.transcriptAnalysis, prompt, defaultResult);
}
...
Write 1-2 sentences. Be specific and personal. Respond with ONLY the explanation text.`;
  ...
  const result = await generateText(genAI, settings.models.recommendationReasoning, prompt);
...
async function extractPreferencesFromFeedback(
  ...
  const keyPointsStr = analysis.keyPoints?.map(kp => `[${kp.seconds}s] ${kp.title}`).join(', ') || 'none';
...
timestampSeconds is optional - include only if preference clearly relates to a specific section`;
  ...
  const extracted = await generateJson<ExtractedPreference[]>(genAI, settings.models.memoryExtraction, prompt, []);
...
{"similarId": "the-id-or-null", "confidence": 0.8, "mergedPreference": "refined preference text"}`;
  ...
  return generateJson<SimilarityResult>(genAI, settings.models.memoryExtraction, prompt, { similarId: null, confidence: 0 });
}
...
async function condenseMemories(
  ...
  const condensedDislikes = await findAndMergeSimilar(genAI, settings, dislikes);
...
Respond with ONLY valid JSON:
{
  ...
  const result = await generateJson<GroupResult>(genAI, settings.models.memoryExtraction, prompt, { groups: memories.map((_, i) => [i]), mergedTexts: {} });
    ...
    if (group.length === 1) {
      ...
      for (let i = 1; i < group.length; i++) {
        ...
        for (const source of other.sources) {
          const exists = merged.sources.some(s => s.videoId === source.videoId);
          if (!exists) {
    ...
    try {
      switch (message.type) {
        case MessageType.ANALYZE_VIDEO: {
          ...
          if (cached) {
            sendResponse({ success: true, analysis: cached } satisfies AnalyzeVideoResponse);
            return;
          }
          ...
          sendResponse({ success: true, analysis } satisfies AnalyzeVideoResponse);
          break;
        }
        ...
        case MessageType.SUBMIT_FEEDBACK: {
          const { videoId, videoTitle, feedback, analysis, channelInfo } = message;
          const settings = await getSettings();

          if (!settings.apiKey) {
            sendResponse({ success: false, error: 'API key not configured' } satisfies SubmitFeedbackResponse);
            return;
          }
          ...
          for (const newMem of extractedMemories) {
            ...
            const sameType = existingMemories.filter(m => m.type === newMem.type);
            ...
            if (similarity.similarId && similarity.confidence >= 0.7) {
              ...
              const mergedMem = existingMemories.find(m => m.id === similarity.similarId);
              if (mergedMem) {
                resultMemories.push({
                  ...mergedMem,
          ...
          sendResponse({ success: true, memories: resultMemories } satisfies SubmitFeedbackResponse);
          break;
        }
        ...
        case MessageType.REGENERATE_VIDEO: {
          ...
          sendResponse({ success: true, analysis } satisfies RegenerateVideoResponse);
          break;
        }
        ...
        case MessageType.STORAGE_GET_FOCUS_SCHEDULE: {
          ...
          sendResponse({ schedule } satisfies StorageGetFocusScheduleResponse);
          break;
        }
        ...
        case MessageType.REGENERATE_ABOUT_ME: {
          ...
          try {
            const aboutMe = await synthesizeAboutMe(genAI, settings, memories);
            ...
            sendResponse({ success: true, aboutMe } satisfies RegenerateAboutMeResponse);
          } catch (error) {
        ...
        case MessageType.GET_LIKED_CHANNELS: {
          ...
          const channelList = Object.values(channels).sort((a, b) => b.lastLikedAt - a.lastLikedAt);
          sendResponse({ channels: channelList } satisfies GetLikedChannelsResponse);
          break;
        }
        ...
        case MessageType.REMOVE_LIKED_CHANNEL: {
          ...
          sendResponse({ success: true } satisfies RemoveLikedChannelResponse);
          break;
        }
        ...
        case MessageType.UPDATE_SUBSCRIPTION_STATUS: {
          ...
          sendResponse({ success: true } satisfies UpdateSubscriptionStatusResponse);
          break;
        }
        ...
        case MessageType.CONDENSE_MEMORIES: {
          ...
          try {
            const condensed = await condenseMemories(genAI, settings, memories);
            ...
            sendResponse({
              success: false,
              error: error instanceof Error ? error.message : 'Condense failed',
            } satisfies CondenseMemoriesResponse);
          }
          break;
        }
...
chrome.runtime.onInstalled.addListener(async () => {
  ...
  // Notify all YouTube tabs to re-detect (useful after extension update)
  chrome.tabs.query({ url: '*://www.youtube.com/*' }, (tabs) => {
    for (const tab of tabs) {
      if (tab.id) {
        chrome.tabs.sendMessage(tab.id, { type: 'RE_DETECT' }).catch(() => {
          // Tab may not have content script yet, ignore error
        });
      }
    }
  });
...
chrome.action.onClicked.addListener(async (tab) => {
  if (tab.id && tab.url?.includes('youtube.com')) {
    console.log('VidPulse: Extension icon clicked, sending RE_DETECT to tab', tab.id);
    try {
