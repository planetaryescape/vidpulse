...
import { setupNavigationListener, extractVideoId } from './navigation';
import { injectPanel, removePanel, triggerFeedback, togglePanelVisibility, triggerRegenerate } from './panel';
import { shouldBlock, showGuardian, removeGuardian } from './guardian';
import { initCheckIn, scheduleNextCheckIn, cancelCheckIn } from './checkin';
import { showIntentPrompt, hasIntentSet } from './intent';
import { handlePlaylistPage, cleanupPlaylist } from './playlist';
import { sendMessage, MessageType } from '../shared/messages';
import { getSettings, getSession, startSession, addVideoToSession, endVideoInSession, updateSessionActivity, updateChannelStats, getChannelStats, updateDailyStats } from './storage-proxy';
import type { AnalyzeVideoResponse, CheckApiKeyResponse, GetCachedResponse } from '../shared/messages';
import type { VideoAnalysis, SessionVideo } from '../shared/types';
...
function getChannelInfo(): { channelId: string; channelName: string } | null {
  ...
  const channelMeta = document.querySelector('meta[itemprop="channelId"]') as HTMLMetaElement | null;
  const nameMeta = document.querySelector('span[itemprop="author"] link[itemprop="name"]') as HTMLLinkElement | null;
  if (channelMeta) {
    return {
...
async function updateSessionVideoWithAnalysis(videoId: string, analysis: VideoAnalysis): Promise<void> {
...
async function handleVideoPage(videoId: string): Promise<void> {
  ...
  try {
    ...
    if (response.success && response.analysis) {
      ...
      await injectPanel({ status: 'ready', videoId, analysis: response.analysis });
...
async function handleNavigation(url: string, videoId: string | null): Promise<void> {
  // End previous video in session
