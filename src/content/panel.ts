...
async function findOrCreateContainer(): Promise<{
	container: Element;
	isFloating: boolean;
}> {
	const selectors = [
...
function getVerdictDisplay(verdict: VideoAnalysis["verdict"]): {
	text: string;
	class: string;
} {
	switch (verdict) {
...
function getSessionStats(session: SessionData): {
	duration: string;
	videoCount: number;
...
function getChannelTrustBadge(stats: ChannelStats): {
	text: string;
	class: string;
} {
	const avgScore =
...
function createFeedbackButtons(state: PanelState): HTMLElement {
	const container = document.createElement("div");
