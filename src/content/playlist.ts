...
import type { AnalyzeVideoResponse, GetCachedResponse } from '../shared/messages';
...
export function isPlaylistPage(): boolean {
  return window.location.pathname === '/playlist' && window.location.search.includes('list=');
}
...
function extractPlaylistVideos(): PlaylistVideo[] {
  ...
  videoElements.forEach((el) => {
    ...
    const durationEl = el.querySelector('span.ytd-thumbnail-overlay-time-status-renderer') as HTMLSpanElement;
...
function getVerdictClass(verdict: string): string {
  switch (verdict) {
    case 'worth_it': return 'vp-verdict-worth';
    case 'maybe': return 'vp-verdict-maybe';
    case 'skip': return 'vp-verdict-skip';
    default: return '';
  }
}
...
function buildPlaylistPanel(analysis: PlaylistAnalysis): HTMLElement {
  ...
  if (analysis.analyzed < analysis.total) {
    ...
    const worthIt = analysis.videos.filter(v => v.analysis?.verdict === 'worth_it').length;
    const maybe = analysis.videos.filter(v => v.analysis?.verdict === 'maybe').length;
    const skip = analysis.videos.filter(v => v.analysis?.verdict === 'skip').length;

    const avgProductivity = Math.round(
      analysis.videos.reduce((sum, v) => sum + (v.analysis?.scores.productivity || 0), 0) / analysis.total
    );
  ...
  const sortedVideos = [...analysis.videos].sort((a, b) => {
    ...
    return (b.analysis?.scores.productivity || 0) - (a.analysis?.scores.productivity || 0);
  });
  ...
  sortedVideos.forEach(video => {
    ...
    thumbEl.src = video.thumbnail || `https://i.ytimg.com/vi/${video.videoId}/default.jpg`;
    thumbEl.alt = '';
  ...
  if (analysis.analyzed === analysis.total) {
    ...
    const worthItFirst = sortedVideos.find(v => v.analysis?.verdict === 'worth_it');
    if (worthItFirst) {
