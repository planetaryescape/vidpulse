...
function getSessionSummary(session: SessionData): string {
	...
	const parts: string[] = [];
	if (categories.educational > 0)
		parts.push(`${categories.educational} educational`);
	if (categories.entertainment > 0)
		parts.push(`${categories.entertainment} entertainment`);
	if (categories.productive > 0)
		parts.push(`${categories.productive} productive`);
