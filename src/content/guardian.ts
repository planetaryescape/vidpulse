import type { Settings, VideoAnalysis } from "../shared/types";
...
export async function shouldBlock(
	...
	const matchedBlockedTags = analysis.tags.filter((tag) =>
		settings.blockedTags.some((blocked) =>
			tag.toLowerCase().includes(blocked.toLowerCase()),
		),
	);
	...
	const threshold = inFocusPeriod
		? focusSchedule.focusThreshold
	...
	if (inFocusPeriod && focusSchedule.blockEntertainment) {
		const isEntertainmentDominant =
			entertainment > productivity && entertainment > educational;
...
export function showGuardian(
	...
	if (blockInfo.reason === "blocked_tags") {
		message.textContent =
			"This video contains content you asked to be warned about.";
	} else if (blockInfo.reason === "focus_entertainment") {
		message.textContent = "Focus mode is blocking entertainment content.";
	} else {
		message.textContent =
			"This video scored below your minimum threshold in all categories.";
	}
	...
	if (blockInfo.reason === "blocked_tags" && blockInfo.blockedTags) {
		...
		focusMsg.textContent =
			"Entertainment content is restricted during focus hours. This video is primarily entertainment.";
