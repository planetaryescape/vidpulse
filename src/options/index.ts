...
const braveApiKeyInput = document.getElementById(
	"braveApiKey",
) as HTMLInputElement;
...
const modelReasoningSelect = document.getElementById(
	"modelReasoning",
) as HTMLSelectElement;
const modelTagsSelect = document.getElementById(
	"modelTags",
) as HTMLSelectElement;
const modelAnalysisSelect = document.getElementById(
	"modelAnalysis",
) as HTMLSelectElement;
const modelMemorySelect = document.getElementById(
	"modelMemory",
) as HTMLSelectElement;
const saveModelsBtn = document.getElementById(
	"saveModelsBtn",
) as HTMLButtonElement;
const modelsStatusEl = document.getElementById(
	"modelsStatus",
) as HTMLDivElement;

// DOM elements - Personalization
const aboutMeTextarea = document.getElementById(
	"aboutMe",
) as HTMLTextAreaElement;
const aboutMeStatusEl = document.getElementById(
	"aboutMeStatus",
) as HTMLDivElement;
const savePersonalizationBtn = document.getElementById(
	"savePersonalization",
) as HTMLButtonElement;
const resetAutoSyncBtn = document.getElementById(
	"resetAutoSync",
) as HTMLButtonElement;
const personalizationStatusEl = document.getElementById(
...
const refreshMemoriesBtn = document.getElementById(
	"refreshMemories",
) as HTMLButtonElement;
const condenseMemoriesBtn = document.getElementById(
	"condenseMemories",
) as HTMLButtonElement;
const clearMemoriesBtn = document.getElementById(
	"clearMemories",
) as HTMLButtonElement;
const memoryStatusEl = document.getElementById(
	"memoryStatus",
) as HTMLDivElement;

// DOM elements - Liked Channels
const likedChannelsCountEl = document.getElementById(
	"likedChannelsCount",
) as HTMLDivElement;
const likedChannelsListEl = document.getElementById(
	"likedChannelsList",
) as HTMLDivElement;
const refreshLikedChannelsBtn = document.getElementById(
	"refreshLikedChannels",
) as HTMLButtonElement;
...
const guardianEnabledCheckbox = document.getElementById(
	"guardianEnabled",
) as HTMLInputElement;
const minScoreThresholdInput = document.getElementById(
	"minScoreThreshold",
) as HTMLInputElement;
const blockedTagsInput = document.getElementById(
	"blockedTags",
) as HTMLInputElement;
const saveGuardianBtn = document.getElementById(
	"saveGuardian",
) as HTMLButtonElement;
const guardianStatusEl = document.getElementById(
	"guardianStatus",
) as HTMLDivElement;

// DOM elements - Check-in
const checkInEnabledCheckbox = document.getElementById(
	"checkInEnabled",
) as HTMLInputElement;
const checkInIntervalSelect = document.getElementById(
	"checkInInterval",
) as HTMLSelectElement;
const saveCheckInBtn = document.getElementById(
	"saveCheckIn",
) as HTMLButtonElement;
...
const notesVideoListEl = document.getElementById(
	"notesVideoList",
) as HTMLDivElement;
const notesDetailEl = document.getElementById("notesDetail") as HTMLDivElement;
const refreshNotesBtn = document.getElementById(
	"refreshNotes",
) as HTMLButtonElement;
const exportAllNotesBtn = document.getElementById(
	"exportAllNotes",
) as HTMLButtonElement;
const notesStatusEl = document.getElementById("notesStatus") as HTMLDivElement;

// DOM elements - Focus Schedule
const focusEnabledCheckbox = document.getElementById(
	"focusEnabled",
) as HTMLInputElement;
const focusDay0Checkbox = document.getElementById(
	"focusDay0",
) as HTMLInputElement;
const focusDay1Checkbox = document.getElementById(
	"focusDay1",
) as HTMLInputElement;
const focusDay2Checkbox = document.getElementById(
	"focusDay2",
) as HTMLInputElement;
const focusDay3Checkbox = document.getElementById(
	"focusDay3",
) as HTMLInputElement;
const focusDay4Checkbox = document.getElementById(
	"focusDay4",
) as HTMLInputElement;
const focusDay5Checkbox = document.getElementById(
	"focusDay5",
) as HTMLInputElement;
const focusDay6Checkbox = document.getElementById(
	"focusDay6",
) as HTMLInputElement;
const focusStartHourSelect = document.getElementById(
	"focusStartHour",
) as HTMLSelectElement;
const focusEndHourSelect = document.getElementById(
	"focusEndHour",
) as HTMLSelectElement;
const focusThresholdInput = document.getElementById(
	"focusThreshold",
) as HTMLInputElement;
const focusBlockEntertainmentCheckbox = document.getElementById(
	"focusBlockEntertainment",
) as HTMLInputElement;
...
function updateAboutMeStatus(autoGenerated: boolean, syncing = false): void {
	...
	if (syncing) {
		...
		aboutMeStatusEl.appendChild(
			document.createTextNode(" Syncing with memories..."),
		);
...
async function renderMemories(): Promise<void> {
	...
	if (memories.length === 0) {
		...
		empty.textContent =
			"No preferences learned yet. Like or dislike videos to teach VidPulse your taste!";
...
async function renderLikedChannels(): Promise<void> {
	try {
		...
		if (channels.length === 0) {
			...
			empty.className = "memory-empty";
			empty.textContent =
				"No liked channels yet. Like videos to track channels!";
		...
		for (const channel of channels) {
			...
			if (channel.avgScores) {
				const avg = Math.round(
					(channel.avgScores.productivity +
						channel.avgScores.educational +
						channel.avgScores.entertainment) /
						3,
				);
...
async function loadSettings(): Promise<void> {
	...
	focusBlockEntertainmentCheckbox.checked =
		focusSchedule.blockEntertainment ?? true;
...
async function handleClearMemories(): Promise<void> {
	if (
		!confirm(
			"Are you sure you want to clear all learned preferences? This cannot be undone.",
		)
	) {
		return;
	}
	...
	try {
		...
		if (response.success) {
			...
			if (reduced > 0) {
				showStatus(
					memoryStatusEl,
					`Condensed ${response.before} → ${response.after} (${reduced} merged)`,
					"success",
				);
			} else {
				showStatus(
					memoryStatusEl,
					"No similar preferences found to merge",
					"success",
				);
			}
			renderMemories();
			...
			showStatus(
				memoryStatusEl,
				response.error || "Failed to condense",
				"error",
			);
		}
	} catch (error) {
...
async function renderDigest(): Promise<void> {
	...
	if (allStats.length === 0) {
		...
		empty.textContent =
			"No watch data yet. Start watching YouTube with VidPulse to see your stats!";
	...
	const weeklyEducational = weeklyStats.reduce(
		(sum, s) => sum + s.byCategory.educational.count,
		0,
	);
	const weeklyEntertainment = weeklyStats.reduce(
		(sum, s) => sum + s.byCategory.entertainment.count,
		0,
	);
	const weeklyProductive = weeklyStats.reduce(
		(sum, s) => sum + s.byCategory.productive.count,
		0,
	);
	const weeklyInspiring = weeklyStats.reduce(
		(sum, s) => sum + s.byCategory.inspiring.count,
		0,
	);
	const weeklyCreative = weeklyStats.reduce(
		(sum, s) => sum + s.byCategory.creative.count,
		0,
	);
...
async function renderNotesDetail(
	videoId: string,
	videoTitle: string,
	videoUrl: string,
): Promise<void> {
...
chrome.storage.onChanged.addListener((changes, areaName) => {
	if (areaName === "sync" && changes.settings) {
		...
		if (newSettings) {
			...
			if (
				newSettings.aboutMeAutoGenerated &&
				newSettings.aboutMe !== aboutMeTextarea.value
			) {
